...
- ### 動かしてみよう！
  - ### ①位置情報を変数にまとめよう
    ...
    せっかく辞書型にマトマたので、draw関数で表示する部分で、この変数を使右葉に変更しよう。
    現時点での全体のコードはこんな感じ
    ```python 
    import pyxel
    pyxel.init(120, 120)
  ...
  - ### ②Xの情報を更新（UPDATE）してみよう
    ...
    **なんかのびたんだがｗｗｗｗ**
    ...
    `pyxle.cls(0)`という一文を加えることで、こんな感じで移動するアニメーションができるよ。ちなみに、oはo番目のカラーコード、つまり黒のこと。
  ...
  - ### 画面外に消えちゃう問題
    ...
    もっというと
    `x軸が〇〇より小さいときは移動する　そうじゃないときは移動しない`
    ってすれば、行けそうな気がする！
    ｋっともっというと、
    `＜もし＞x軸が〇〇より小さいときは移動する　＜そうじゃないとき＞は移動しない`
